...
**插件绝不读取你的工作区内容。** 它不会查看会话标题、消息正文、文件路径、工具输入输出或任何其他内容，只响应粗粒度的生命周期信号（`agent/inbox/inserted`、`agent/status`、`tools/pre-execute`、`session/created`、`workflow/start`），然后推送配置好的状态字符串。即使保持默认列表，Discord 上也只会出现"正在指挥大肥鱼干活"这样的模糊状态。
...
发布包：<https://github.com/0QwQ0/dsh-discord-richpresence/releases/latest/download/dsh-discord-richpresence-0.1.0.tgz>
...
- `lib/index.js` — Cordis 宿主插件。注册粗粒度宿主事件的全局监听器，映射到配置好的状态列表，再通过 RPC 客户端推送。插件纤维卸载时会清理所有定时器并关闭 socket。
