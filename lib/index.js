...
/**
 * Settings namespace the web row writes.
 *
 * Deliberately spelled as a plain lowercase-hyphenated string: the harness
 * validates namespace ids against `/^[a-z][a-z0-9-]*$/`.
 */
const SETTINGS_NS = 'discord-richpresence'

/**
 * Register the rich-mode settings section against every settings API
 * generation this plugin supports.
 *
 * The settings capability moved between harness releases:
 * - 0.1.5+ exposes `SettingsProvider.installSection(owner, ns, schema, entry, hooks)`
 *   (`ctx.settings`), and dropped the module-level `installSettingsSection` /
 *   `settingsNamespace` helpers that 0.1.1 shipped.
 * - 0.1.1 only has `SettingsProvider.register(ns, schema, { base })`.
 *
 * `register` exists in both generations with the same signature, so the older
 * path is a faithful fallback: it resolves the namespace over the composition
 * entry as `base` and observes committed changes through `watch`.
 *
 * The whole wiring runs inside `ctx.inject(['settings'])`, so the plugin works
 * with or without a settings service and re-attaches if one appears later.
 *
 * @param {import('@deepseek-ai/cordis').Context} ctx - the plugin fiber context (also the section owner).
 * @param {object} schema - schemastery schema resolving the namespace value.
 * @param {object} entry - composition entry value used as the `base` layer.
 * @param {{ setSource: (current: () => object) => void, onChange: () => void }} hooks - source sink and change notification.
 */
function installRichModeSection(ctx, schema, entry, hooks) {
  /** Attach to a live settings provider using whichever API it exposes. */
  const attach = (settings, ownerCtx) => {
    if (typeof settings?.installSection === 'function') {
      // 0.1.5+: the provider owns base/fallback semantics and change fan-out.
      settings.installSection(ownerCtx, SETTINGS_NS, schema, entry, hooks)
      return
    }
    if (typeof settings?.register === 'function') {
      // 0.1.1 fallback: resolve over `base` and watch committed changes.
      const scope = settings.register(SETTINGS_NS, schema, { base: entry })
    ...
    // No usable settings provider: keep operating on the composition entry.
    hooks.setSource(() => entry)
    hooks.onChange()
  }

  const live = ctx.get('settings')
  if (live !== undefined) {
    attach(live, ctx)
    return
  }
  ctx.inject(['settings'], (settingsCtx) => {
    attach(settingsCtx.settings, settingsCtx)
  })
}
...
export function apply(ctx, config = {}) {
  ...
  installRichModeSection(ctx, z.object({
